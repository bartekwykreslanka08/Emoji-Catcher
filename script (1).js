...
let magnetTimeout;
let currentModifier = '';
let secondPlayerPosition = 65;

const modes = {
    easy: { initialTime: 50, goodPoints: [4, 5, 6, 5, 4, 6, 5, 4], goodTime: [5, 6, 8, 6, 5, 7, 6, 5], badPoints: [-5], badTime: [-6], emojiInterval: 600, bombChance: 0.3, magnetChance: 0.05 },
    medium: { initialTime: 30, goodPoints: [5, 6, 7, 6, 5, 7, 6, 5], goodTime: [6, 7, 9, 7, 6, 8, 7, 6], badPoints: [-6], badTime: [-7], emojiInterval: 450, bombChance: 0.4, magnetChance: 0.03 },
    hard: { initialTime: 15, goodPoints: [6, 7, 8, 7, 6, 8, 7, 6], goodTime: [7, 8, 10, 8, 7, 9, 8, 7], badPoints: [-7], badTime: [-8], emojiInterval: 300, bombChance: 0.5, magnetChance: 0.01 }
};
...
window.onload = () => {
    ...
    setTimeout(() => {
        ...
        const savedFont = localStorage.getItem('font_' + location.hostname) || 'default';
...
function logout() {
    ...
    cancelAnimationFrame(waveLoop);
    document.getElementById('score').textContent = '0';
    document.getElementById('time').textContent = '30';
    document.getElementById('waveTimer').textContent = '30';
    gameStarted = false;
    isPaused = false;
...
function showTopScores() {
    ...
    ['easy', 'medium', 'hard'].forEach(mode => {
        ...
        if (modeScores.length > 0) {
            topScoresList.innerHTML += `<h3 style="color: #8a3aff; font-size: 4.5vw;">${mode.charAt(0).toUpperCase() + mode.slice(1)}:</h3><ul>`;
...
function startGame() {
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        if (currentModifier === 'duet') {
            gameArea.innerHTML += '<div class="player" style="transform: translateX(35vw);">🧺</div>';
            gameArea.innerHTML += '<div class="second-player" style="transform: translateX(65vw);">🧺</div>';
        } else {
            gameArea.innerHTML += `<div class="player ${currentModifier === 'biggerBasket' ? 'bigger-player' : ''}" style="transform: translateX(35vw);">🧺</div>`;
        }
        ...
        function gameLoopTimestamp(timestamp) {
            if (!isPaused) {
                ...
                if (!isWaveActive && currentModifier !== 'oneMinute') {
                    time -= deltaTime;
                    waveTimer -= deltaTime;
                    document.getElementById('time').textContent = Math.max(0, Math.round(time * 10) / 10);
                    document.getElementById('waveTimer').textContent = Math.max(0, Math.round(waveTimer));
                    if (waveTimer <= 0) {
                        startWave();
                    }
                } else if (currentModifier === 'oneMinute') {
                    time -= deltaTime;
                    document.getElementById('time').textContent = Math.max(0, Math.round(time * 10) / 10);
                }
                if (time <= 0) {
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis) {
                if (time > 0) {
                    ...
                    const goodEmojis = themeEmojis[currentTheme];
                    const bombChance = isWaveActive ? 0.7 : modes[currentMode].bombChance;
                    const magnetChance = modes[currentMode].magnetChance;
                    const emojis = [
                        ...goodEmojis.map((e, i) => ({ emoji: e, index: i, type: 'good' })),
                        { emoji: '💣', index: 0, type: 'bad' }
                    ];
                    const emojiData = Math.random() < bombChance ? { emoji: '💣', index: 0, type: 'bad' } :
                        Math.random() < magnetChance ? { emoji: '🧲', index: 0, type: 'magnet' } :
                        goodEmojis.map((e, i) => ({ emoji: e, index: i, type: 'good' }))[Math.floor(Math.random() * (goodEmojis.length - 1))];
                    ...
                    setTimeout(() => {
                        function animateEmoji() {
                            let top = parseFloat(emoji.dataset.top) + 1.2 * deltaTime * 60;
                            emoji.dataset.top = top;
                            if (magnetActive && emojiData.type !== 'bad') {
                                const player = document.querySelector('.player');
                                if (player) {
                                    const gameAreaRect = gameArea.getBoundingClientRect();
                                    const playerRect = player.getBoundingClientRect();
                                    const emojiRect = emoji.getBoundingClientRect();
                                    const playerX = (playerRect.left - gameAreaRect.left) / gameAreaRect.width * 100;
                                    const pullStrength = 0.5;
                                    let targetX = playerX - (playerWidth / 2) + (playerWidth * Math.random());
                                    let dx = (targetX - parseFloat(emoji.style.left)) * pullStrength * deltaTime * 60;
                                    emoji.style.left = (parseFloat(emoji.style.left) + dx) + 'vw';
                                }
                            }
                            emoji.style.transform = `translateY(${top}vh)`;
                            if (top > 75) {
                                emoji.remove();
                                emojiCount--;
                            } else {
                                ...
                                if (player || secondPlayer) {
                                    ...
                                    const checkCollision = (rect) => {
                                        if (rect) {
                                            const playerX = (rect.left - gameAreaRect.left) / gameAreaRect.width * 100;
                                    ...
                                    if ((player && checkCollision(playerRect)) || (secondPlayer && checkCollision(secondPlayerRect))) {
                                        ...
                                        if (emojiData.type === 'magnet') {
                                            ...
                                            const points = emojiData.type === 'good' ? modeData.goodPoints[emojiData.index % modeData.goodPoints.length] : modeData.badPoints[0];
                                            const timeChange = emojiData.type === 'good' ? modeData.goodTime[emojiData.index % modeData.goodTime.length] : modeData.badTime[0];
                                            score += points;
                                            time += timeChange / 10;
                                            document.getElementById('score').textContent = score;
                                            document.getElementById('time').textContent = Math.max(0, Math.round(time * 10) / 10);
...
function startWave() {
    isWaveActive = true;
    document.getElementById('waveDisplay').textContent = 'Fala: 10s';
    setTimeout(endWave, 10000);
}

function endWave() {
    isWaveActive = false;
    waveTimer = 30;
    document.getElementById('waveDisplay').textContent = `Fala: ${waveTimer}s`;
}

function changeFont(font) {
    document.body.style.fontFamily = font === 'default' ? "'Segoe UI', 'Roboto', sans-serif" : font;
    localStorage.setItem('font_' + location.hostname, font);
}