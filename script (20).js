...
const moveSpeed = 15;
const magnetRadius = 25; // Zwiększony promień magnesu
const magnetPullStrength = 5; // Zwiększona siła przyciągania
...
function startGame() {
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        document.querySelector('.game-stats').innerHTML = `
            <div class="stat-item" id="scoreDisplay">Punkty: <span id="score">${score}</span></div>
            <div class="stat-item" id="timeDisplay">Czas: <span id="time">${time}</span>s</div>
            <button id="stopButton" onclick="stopGame()">X</button>
        `;
        gameStarted = true;
        isPaused = false;
                            ...
                            if (player || secondPlayer) {
                                ...
                                if (magnetActive) {
                                    ...
                                    if (distance <= magnetRadius) {
                                        const pullSpeed = magnetPullStrength * (magnetRadius - distance) * deltaTime; // Mocniejsze przyciąganie
                                        if (emojiX < playerX) {
                                ...
                                if ((player && checkCollision(playerRect)) || (secondPlayer && checkCollision(secondPlayerRect))) {
                                    ...
                                    if (emojiData.type === 'magnet') {
                                        ...
                                        score += 10;
                                        document.getElementById('score').textContent = score;
                                        magnetTimeout = setTimeout(() => { magnetActive = false; }, 5000); // 5 sekund
                                    } else {
