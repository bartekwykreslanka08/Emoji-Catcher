...
const moveSpeed = 15;
const magnetRadius = 40; // Zwiększono zasięg magnesu
const magnetPullStrength = 15; // Zwiększono siłę przyciągania
                    ...
                    function animateEmoji() {
                        ...
                        if (top > 75) {
                            ...
                            if (player || secondPlayer) {
                                ...
                                const checkCollision = (pRect, pPos) => {
                                    if (pRect) {
                                ...
                                if (magnetActive) {
                                    ...
                                    const distanceToPlayer = Math.abs(emojiX - playerX);
                                    const distanceToSecondPlayer = Math.abs(emojiX - secondPlayerX);

                                    if (distanceToPlayer <= magnetRadius || (secondPlayer && distanceToSecondPlayer <= magnetRadius)) {
                                        const pullSpeed = magnetPullStrength * deltaTime;
                                        if (distanceToPlayer <= magnetRadius) {
                                            const pullDirection = (playerX - emojiX) / distanceToPlayer;
                                            emoji.style.left = `${Math.max(0, Math.min(gameWidth - 7, parseFloat(emoji.style.left) + pullDirection * pullSpeed))}vw`;
                                        }
                                        if (secondPlayer && distanceToSecondPlayer <= magnetRadius) {
                                            const pullDirectionSecond = (secondPlayerX - emojiX) / distanceToSecondPlayer;
                                            emoji.style.left = `${Math.max(0, Math.min(gameWidth - 7, parseFloat(emoji.style.left) + pullDirectionSecond * pullSpeed))}vw`;
                                        }
                                    }
                                }

                                if ((player && checkCollision(playerRect, playerPosition)) || (secondPlayer && checkCollision(secondPlayerRect, secondPlayerPosition))) {
