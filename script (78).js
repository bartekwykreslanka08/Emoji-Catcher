...
const magnetPullStrength = 0.7; // Zwiększona siła dla płynniejszego działania
...
function toggleTopScores() {
    ...
    if (topScoresSection.style.display === 'block') {
                        ...
                        if (player || secondPlayer) {
                            ...
                            if (magnetActive) {
                                ...
                                if (minDistance <= magnetRadius) {
                                    const pullSpeed = magnetPullStrength * (1 - minDistance / magnetRadius) * deltaTime * 200; // Zwiększona częstotliwość dla płynności
