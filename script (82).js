    ...
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis && time > 0) {
                ...
                if (currentModifier === 'duet') {
                    if (Math.abs(emojiX - playerPosition) < playerWidth / 2 || (secondPlayerRect && Math.abs(emojiX - secondPlayerPosition) < playerWidth / 2)) {
                        handleCollision(emojiData, emoji);
                    }
                } else if (Math.abs(emojiX - playerPosition) < (currentModifier === 'miniBasket' ? playerWidth / 2 : currentModifier === 'biggerBasket' ? playerWidth * 1.5 / 2 : playerWidth / 2)) {
                    handleCollision(emojiData, emoji);
                }
            }
...
function stopGame() {
    isPaused = !isPaused;
    document.getElementById('stopButton').textContent = isPaused ? 'Wznów' : 'Stop';
}
...
function handleCollision(emojiData, emoji) {
    if (emojiData.type === 'good') {
    ...
    if (magnetActive) {
        ...
        emojis.forEach(emoji => {
            ...
            if (distance < magnetRadius) {
                ...
                const collisionDistance = currentModifier === 'biggerBasket' ? playerWidth * 1.5 / 2 : currentModifier === 'miniBasket' ? playerWidth / 2 : playerWidth / 2;
                if (Math.abs(emojiX - playerPosition) < collisionDistance) {
                    const emojiData = {
                        emoji: emoji.textContent,
                        type: themeEmojis[currentTheme].includes(emoji.textContent) && emoji.textContent !== '🧲' ? 'good' : emoji.textContent === '💣' ? 'bad' : 'magnet'
                    };
                    handleCollision(emojiData, emoji);
                }
            }
