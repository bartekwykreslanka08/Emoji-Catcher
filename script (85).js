    ...
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        function gameLoopTimestamp(timestamp) {
            if (!isPaused) {
                ...
                checkCollisions();
                updateEmojiPositions(timestamp); // Aktualizacja pozycji emoji w czasie rzeczywistym
            }
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis && time > 0) {
                ...
                emoji.dataset.duration = modes[currentMode].emojiInterval * 1.5; // Trwanie animacji dostosowane do trybu
...
function updateEmojiPositions(timestamp) {
    const emojis = document.querySelectorAll('.emoji');
    emojis.forEach(emoji => {
        ...
        const newY = -10 + progress * (height + 10); // Od -10vh (góra) do height + 10vh (dół)
        emoji.style.transform = `translateY(${newY}px)`;
        if (progress >= 1) {
            if (emoji.parentElement) {
                emoji.parentElement.removeChild(emoji);
                emojiCount--;
