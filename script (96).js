...
const modes = {
    easy: { initialTime: 50, goodPoints: [4, 5, 6, 5, 4, 6, 5, 4, 6, 5, 4, 6, 5], goodTime: [5, 6, 8, 6, 5, 7, 6, 5, 7, 6, 5, 8, 6], badPoints: [-5], badTime: [-6], emojiInterval: 800, fallDuration: 10000, bombChance: 0.2, magnetChance: 0.1 },
    medium: { initialTime: 30, goodPoints: [5, 6, 7, 6, 5, 7, 6, 5, 7, 6, 5, 7, 6], goodTime: [6, 7, 9, 7, 6, 8, 7, 6, 8, 7, 6, 9, 7], badPoints: [-6], badTime: [-7], emojiInterval: 600, fallDuration: 7500, bombChance: 0.4, magnetChance: 0.08 },
    hard: { initialTime: 15, goodPoints: [6, 7, 8, 7, 6, 8, 7, 6, 8, 7, 6, 9, 7], goodTime: [7, 8, 10, 8, 7, 9, 8, 7, 9, 8, 7, 10, 8], badPoints: [-7], badTime: [-8], emojiInterval: 400, fallDuration: 5000, bombChance: 0.6, magnetChance: 0.06 }
};
...
const magnetPullStrength = 3.0; // Ulepszony magnes, większa siła i płynność
...
function startGame() {
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        function gameLoopTimestamp(timestamp) {
            if (!isPaused) {
                deltaTime = Math.min(0.1, (timestamp - lastTime) / 1000); // Ograniczenie deltaTime dla płynności
                lastTime = timestamp;
                time -= deltaTime;
        ...
        for (let emoji of emojis) {
            if (!emoji.classList.contains('magnet-effect')) { // Pomijamy sam magnes
                ...
                if (distance < magnetRadius) {
                    const pullStrength = magnetPullStrength * (1 - distance / magnetRadius) * deltaTime * 100; // Szybkie i płynne przyciąganie
