...
const modes = {
    easy: { initialTime: 50, goodPoints: [4, 5, 6, 5, 4, 6, 5, 4, 6, 5, 4, 6, 5], goodTime: [5, 6, 8, 6, 5, 7, 6, 5, 7, 6, 5, 8, 6], badPoints: [-5], badTime: [-6], emojiInterval: 800, fallDuration: 3000, bombChance: 0.2, specialChance: 0.05 },
    medium: { initialTime: 30, goodPoints: [5, 6, 7, 6, 5, 7, 6, 5, 7, 6, 5, 7, 6], goodTime: [6, 7, 9, 7, 6, 8, 7, 6, 8, 7, 6, 9, 7], badPoints: [-5], badTime: [-6], emojiInterval: 600, fallDuration: 2000, bombChance: 0.3, specialChance: 0.05 },
    hard: { initialTime: 15, goodPoints: [6, 7, 8, 7, 6, 8, 7, 6, 8, 7, 6, 9, 7], goodTime: [7, 8, 10, 8, 7, 9, 8, 7, 9, 8, 7, 10, 8], badPoints: [-5], badTime: [-6], emojiInterval: 400, fallDuration: 1500, bombChance: 0.4, specialChance: 0.05 },
    'hard+': { initialTime: 10, goodPoints: [6, 7, 8, 7, 6, 8, 7, 6, 8, 7, 6, 9, 7], goodTime: [7, 8, 10, 8, 7, 9, 8, 7, 9, 8, 7, 10, 8], badPoints: [-5], badTime: [-6], emojiInterval: 300, fallDuration: 1000, bombChance: 0.5, specialChance: 0.05 }
};
    ...
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis && time > 0) {
                ...
                const specialChance = modes[currentMode].specialChance;
                ...
                if (currentModifier === 'disco') {
                    ...
                    if (random < bombChance) {
                        emojiText = '💣';
                    } else if (random < bombChance + specialChance * 0.4) { // 0️⃣ (40% specjalnych)
                        emojiText = '0️⃣';
                    } else if (random < bombChance + specialChance) { // 💲 (60% specjalnych)
                        emojiText = '💲';
                    } else {
...
function handleEmojiCollision(emoji) {
    ...
    if (emojiText === '💣') {
        ...
        resetGame();
        return; // Zatrzymaj dalsze przetwarzanie po resecie
    } else if (currentModifier === 'disco' && emojiText === '🪩') {
        const randomPoints = Math.floor(Math.random() * 21) - 5; // -5 do +15
        const randomTime = Math.floor(Math.random() * 21) - 5; // -5 do +15
        score += randomPoints;
        time += randomTime;
    } else if (currentModifier === 'numbers' && numberEmojis.includes(emojiText)) {
