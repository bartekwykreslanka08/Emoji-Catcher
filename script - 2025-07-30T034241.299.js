...
function startGame() {
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        document.getElementById('time').textContent = `5️⃣ ${time}`;
        updateGameModeInfo();
        ...
        function gameLoopTimestamp(timestamp) {
            if (!isPaused) {
                ...
                lastTime = timestamp;
                time -= deltaTime;
                document.getElementById('time').textContent = `5️⃣ ${Math.max(0, Math.round(time * 10) / 10)}`;
                handleMagnet();
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis && time > 0) {
                ...
                if (random < modes[currentMode].bombChance) {
                    emojiText = '💣';
                } else if (random < modes[currentMode].bombChance + 0.05) { // Gwarantuje pojawianie się 0️⃣
                    emojiText = '0️⃣';
                } else if (random < modes[currentMode].bombChance + 0.10) { // Gwarantuje pojawianie się 💲
                    emojiText = '💲';
                } else if (random < modes[currentMode].bombChance + 0.10 + modes[currentMode].magnetChance) {
                    emojiText = '🧲';
...
function handleTouchMove(e) {
    if (gameStarted && !isPaused) {
        ...
        targetPosition = Math.max(0, Math.min(gameWidth - playerWidth, playerPosition + deltaX * 0.8));
        touchStartX = touchX;
    }
}
    ...
    if (gameStarted && !isPaused) {
        ...
        for (let emoji of emojis) {
            ...
            const adjustedPlayerRect = {
                ...
                bottom: playerRect.bottom - gameAreaRect.top + 2
            };
...
function handleEmojiCollision(emoji) {
    ...
    if (emojiText === '💣') {
        ...
        score = 0;
        time = modes[currentMode].initialTime / 2;
    } else if (emojiText === '💲') {
        score += 50;
        time += 25;
    } else if (emojiText === '🧲') {
        ...
        for (let emoji of emojis) {
            if (!emoji.classList.contains('magnet-effect')) {
                ...
                if (emojiY >= playerY && emojiY <= playerY + magnetHeight) {
                    ...
                    const pullStrength = 15 * deltaTime * 60;
