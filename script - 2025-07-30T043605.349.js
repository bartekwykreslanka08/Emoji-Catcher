...
const modes = {
    easy: { initialTime: 50, goodPoints: [4, 5, 6, 5, 4, 6, 5, 4, 6, 5, 4, 6, 5], goodTime: [5, 6, 8, 6, 5, 7, 6, 5, 7, 6, 5, 8, 6], badPoints: [-5], badTime: [-6], emojiInterval: 800, fallDuration: 3000, bombChance: 0.15, dollarChance: 0.03, zeroChance: 0.04 },
    medium: { initialTime: 30, goodPoints: [5, 6, 7, 6, 5, 7, 6, 5, 7, 6, 5, 7, 6], goodTime: [6, 7, 9, 7, 6, 8, 7, 6, 8, 7, 6, 9, 7], badPoints: [-5], badTime: [-6], emojiInterval: 600, fallDuration: 2000, bombChance: 0.3, dollarChance: 0.02, zeroChance: 0.03 },
    hard: { initialTime: 15, goodPoints: [6, 7, 8, 7, 6, 8, 7, 6, 8, 7, 6, 9, 7], goodTime: [7, 8, 10, 8, 7, 9, 8, 7, 9, 8, 7, 10, 8], badPoints: [-5], badTime: [-6], emojiInterval: 400, fallDuration: 1500, bombChance: 0.45, dollarChance: 0.015, zeroChance: 0.02 },
    hardPlus: { initialTime: 10, goodPoints: [7, 8, 9, 8, 7, 9, 8, 7, 9, 8, 7, 10, 8], goodTime: [8, 9, 11, 9, 8, 10, 9, 8, 10, 9, 8, 11, 9], badPoints: [-5], badTime: [-6], emojiInterval: 300, fallDuration: 1200, bombChance: 0.5, dollarChance: 0.01, zeroChance: 0.015 }
};
...
function startGame() {
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        gameArea.innerHTML = '';
        gameArea.style.position = 'relative'; // Upewnienie się, że pole gry jest renderowane
        gameArea.style.minHeight = '80vh'; // Minimalna wysokość dla widoczności
        gameArea.style.width = '100%'; // Pełna szerokość
        ...
        gameArea.style.display = 'block'; // Upewnienie się, że jest widoczne
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis && time > 0) {
                ...
                const goodEmojis = themeEmojis[currentTheme].filter(e => e !== '💣' && e !== '💲' && e !== '0️⃣');
    ...
    if (gameStarted && !isPaused) {
        ...
        for (let emoji of emojis) {
            ...
            const adjustedPlayerRect = {
                left: playerRect.left - gameAreaRect.left + 1,
                right: playerRect.right - gameAreaRect.left - 1,
                top: playerRect.top - gameAreaRect.top,
                bottom: playerRect.bottom - gameAreaRect.top
            };
            const adjustedEmojiRect = {
                left: emojiRect.left - gameAreaRect.left + 1,
                right: emojiRect.right - gameAreaRect.left - 1,
            ...
            if (currentModifier === 'duet') {
                ...
                const adjustedSecondPlayerRect = {
                    left: secondPlayerRect.left - gameAreaRect.left + 1,
                    right: secondPlayerRect.right - gameAreaRect.left - 1,
...
function handleEmojiCollision(emoji) {
    ...
    if (emojiText === '💣') {
        ...
        const numberValue = parseInt(emojiText.replace(/[^0-9]/g, '')) || 0;
        score += numberValue;
        time += numberValue;
    } else {
        const goodEmojis = themeEmojis[currentTheme].filter(e => e !== '💣' && e !== '💲' && e !== '0️⃣');
        ...
        if (index !== -1) {
            const pointIndex = Math.floor(Math.random() * 3) + 4;
            const timeIndex = Math.floor(Math.random() * 5) + 5;
            score += pointIndex;
            time += timeIndex;
        }
    }
...
function updateEmojiAnimation() {
    ...
    for (let emoji of emojis) {
        ...
        if (rect.bottom >= gameAreaRect.bottom || rect.top > gameAreaRect.bottom) {
            ...
            emoji.style.top = `${Math.min(currentTop, 90)}vh`; // Ograniczenie górnej krawędzi
        }
    }
}
...
function applyTheme(theme) {
    ...
    const backgrounds = {
        default: 'linear-gradient(to bottom, #87CEEB, #FFFFFF)',
        retro: 'linear-gradient(to bottom, #4B0082, #FFD700)',
        ocean: 'linear-gradient(to bottom, #00CED1, #E0FFFF)',
        winter: 'linear-gradient(to bottom, #ADD8E6, #F0FFF0)',
        summer: 'linear-gradient(to bottom, #FFA500, #FFFFE0)',
        color: 'linear-gradient(to bottom, #FF0000, #00FF00, #0000FF)',
        evening: 'linear-gradient(to bottom, #1E1E2F, #8A2BE2)',
        halloween: 'linear-gradient(to bottom, #000000, #800000)',
        neon: 'linear-gradient(to bottom, #00FF7F, #FF00FF)',
        time: 'linear-gradient(to bottom, #D3D3D3, #A9A9A9)',
        food: 'linear-gradient(to bottom, #FFD700, #FF4500)',
        animals: 'linear-gradient(to bottom, #90EE90, #228B22)',
        vehicles: 'linear-gradient(to bottom, #C0C0C0, #696969)'
    };
    document.body.style.background = backgrounds[theme] || 'linear-gradient(to bottom, #87CEEB, #FFFFFF)';
}
