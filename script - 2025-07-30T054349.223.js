...
const modes = {
    ...
    medium: { initialTime: 30, goodPoints: [5, 6, 7], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 600, fallDuration: 2000, bombChance: 0.3, dollarChance: 0.015, zeroChance: 0.025 },
    hard: { initialTime: 15, goodPoints: [6, 7, 8], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 400, fallDuration: 1500, bombChance: 0.45, dollarChance: 0.01, zeroChance: 0.02 },
    hardPlus: { initialTime: 10, goodPoints: [7, 8, 9], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 300, fallDuration: 1200, bombChance: 0.6, dollarChance: 0.008, zeroChance: 0.015 }
};
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis && time > 0) {
                ...
                if (random < modes[currentMode].bombChance) {
                    ...
                    const goodEmojis = themeEmojis[currentTheme].filter(e => e !== '🧲' && e !== '💣' && e !== '💲' && e !== '0️⃣');
...
function handleEmojiCollision(emoji) {
    ...
    if (emojiText === '💣') {
        ...
        const randomPoints = Math.floor(Math.random() * 16) - 5; // -5 do +10
        const randomTime = Math.floor(Math.random() * 16) - 5;   // -5s do +10s
        score += randomPoints;
        time += randomTime;
    } else if (currentModifier === 'numbers' && numberEmojis.includes(emojiText)) {
        const numberValue = parseInt(emojiText.replace(/️⃣/g, ''));
        score += numberValue;
        time += numberValue;
    } else {
