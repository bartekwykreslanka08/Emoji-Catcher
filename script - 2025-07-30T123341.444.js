...
const modes = {
    easy: { initialTime: 50, goodPoints: [4, 5, 6], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 800, fallDuration: 3000, bombChance: 0.15, dollarChance: 0.02, zeroChance: 0.025 },
    medium: { initialTime: 30, goodPoints: [5, 6, 7], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 600, fallDuration: 2000, bombChance: 0.3, dollarChance: 0.015, zeroChance: 0.02 },
    hard: { initialTime: 15, goodPoints: [6, 7, 8], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 400, fallDuration: 1500, bombChance: 0.45, dollarChance: 0.01, zeroChance: 0.015 },
    hardPlus: { initialTime: 10, goodPoints: [7, 8, 9], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 300, fallDuration: 1200, bombChance: 0.6, dollarChance: 0.008, zeroChance: 0.012 }
};
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis && time > 0) {
                ...
                if (random < modes[currentMode].bombChance) {
                    ...
                    const goodEmojis = (currentModifier === 'disco' ? ['🪩'] : themeEmojis[currentTheme]).filter(e => e !== '🧲');
                ...
                emoji.style.top = '-6vh';
                emoji.style.transition = `top ${modes[currentMode].fallDuration / 1000 / 60}s linear, opacity 0.1s ease`;
...
function handleEmojiCollision(emoji) {
    ...
    if (emojiText === '💣') {
        ...
        const numberValue = parseInt(emojiText.replace(/\D/g, '')) || 0;
        score += numberValue;
        time += numberValue;
    } else {
