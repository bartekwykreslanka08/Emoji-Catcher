...
const modes = {
    easy: { initialTime: 50, goodPoints: [4, 5, 6], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 800, fallDuration: 3000, bombChance: 0.1, specialChance: 0.05 },
    medium: { initialTime: 30, goodPoints: [5, 6, 7], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 600, fallDuration: 2000, bombChance: 0.2, specialChance: 0.05 },
    hard: { initialTime: 15, goodPoints: [6, 7, 8], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 400, fallDuration: 1500, bombChance: 0.3, specialChance: 0.05 },
    hardPlus: { initialTime: 10, goodPoints: [7, 8, 9], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 300, fallDuration: 1000, bombChance: 0.4, specialChance: 0.05 }
};
...
function startGame() {
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        } else if (currentModifier === 'biggerBasket') {
            gameArea.innerHTML += '<div class="player bigger-player" style="transform: translateX(35vw); bottom: 2vh;">🧺</div>';
        } else {
            gameArea.innerHTML += '<div class="player" style="transform: translateX(35vw); bottom: 2vh;">🧺</div>';
        }
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis && time > 0) {
                ...
                if (random < modes[currentMode].bombChance) {
                    emojiText = '💣';
                } else if (random < modes[currentMode].bombChance + 0.02) { // 2% na 💲
                    emojiText = '💲';
                } else if (random < modes[currentMode].bombChance + 0.05) { // 3% na 0️⃣
                    emojiText = '0️⃣';
                ...
                emoji.style.top = '-6vh';
                emoji.style.transition = `transform ${modes[currentMode].fallDuration / 1000}s linear, opacity 0.1s ease`;
...
function handleEmojiCollision(emoji) {
    ...
    if (emojiText === '💣') {
        ...
        const randomTime = Math.floor(Math.random() * 7) - 3; // -3 do +3
        score += randomPoints;
        time += randomTime;
    } else if (currentModifier === 'numbers') {
        ...
        if (index !== -1) {
            const pointIndex = Math.floor(Math.random() * 3) + 4; // 4-6
            const timeIndex = Math.floor(Math.random() * 5) + 5; // 5-9
            score += pointIndex;
            time += timeIndex;
        }
    }
        ...
        for (let emoji of emojis) {
            if (!emoji.classList.contains('magnet-effect')) {
                ...
                if (emojiY >= playerY && emojiY <= playerY + magnetHeight) {
                    ...
                    emoji.style.transform = `translateX(${Math.max(0, Math.min(gameWidth - 6, newX))}vw)`;
                }
            }
        }
    }
...
function updateEmojiAnimation() {
    ...
    for (let emoji of emojis) {
        ...
        if (rect.bottom >= gameAreaRect.bottom) {
            ...
            emoji.style.transform = `translateY(${currentTop}vh)`;
        }
    }
}
