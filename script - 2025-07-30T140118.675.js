...
const modes = {
    easy: { initialTime: 50, goodPoints: [4, 5, 6], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 800, fallDuration: 3000, bombChance: 0.15, specialChance: 0.02, resetChance: 0.025 },
    medium: { initialTime: 30, goodPoints: [5, 6, 7], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 600, fallDuration: 2000, bombChance: 0.3, specialChance: 0.015, resetChance: 0.02 },
    hard: { initialTime: 15, goodPoints: [6, 7, 8], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 400, fallDuration: 1500, bombChance: 0.45, specialChance: 0.01, resetChance: 0.015 },
    hardPlus: { initialTime: 10, goodPoints: [7, 8, 9], goodTime: [5, 6, 7, 8, 9], badPoints: [-6], badTime: [-8], emojiInterval: 300, fallDuration: 1200, bombChance: 0.6, specialChance: 0.008, resetChance: 0.012 }
};
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis && time > 0) {
                ...
                if (random < modes[currentMode].bombChance) {
                    emojiText = '💣';
                } else if (random < modes[currentMode].bombChance + modes[currentMode].specialChance) {
                    emojiText = '💲';
                } else if (random < modes[currentMode].bombChance + modes[currentMode].specialChance + modes[currentMode].resetChance) {
                    emojiText = '0️⃣';
                ...
                emoji.style.top = '-12vh'; // Ustawienie startu poza górną krawędzią ekranu
...
function handleEmojiCollision(emoji) {
    ...
    if (emojiText === '💣') {
        ...
        const value = parseInt(emojiText.replace(/[^0-9]/g, '')) || 0;
        score += value;
        time += value;
    } else {
...
function updateEmojiAnimation() {
    ...
    for (let emoji of emojis) {
        ...
        if (rect.bottom >= gameAreaRect.bottom) {
            ...
            let currentTop = parseFloat(emoji.style.top) || -12; // Upewnienie się, że startuje od -12vh
