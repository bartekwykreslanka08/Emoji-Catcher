...
const modes = {
    easy: { initialTime: 50, goodPoints: [4, 5, 6], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 800, fallDuration: 3000, bombChance: 0.16, magnetChance: 0.08 },
    medium: { initialTime: 30, goodPoints: [5, 6, 7], goodTime: [6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 600, fallDuration: 2000, bombChance: 0.32, magnetChance: 0.06 },
    hard: { initialTime: 15, goodPoints: [6, 7, 8], goodTime: [7, 8, 9, 10], badPoints: [-5], badTime: [-6], emojiInterval: 400, fallDuration: 1500, bombChance: 0.48, magnetChance: 0.05 },
    hardPlus: { initialTime: 10, goodPoints: [7, 8, 9], goodTime: [8, 9, 10, 11], badPoints: [-5], badTime: [-6], emojiInterval: 300, fallDuration: 1200, bombChance: 0.6, magnetChance: 0.04 }
};
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis && time > 0) {
                ...
                if (random < modes[currentMode].bombChance) {
                    emojiText = '💣';
                } else if (random < modes[currentMode].bombChance + 0.03) { // 0️⃣ with slightly higher chance than 💲
                    emojiText = '0️⃣';
                } else if (random < modes[currentMode].bombChance + 0.03 + 0.02) { // 💲 with lower chance
                    emojiText = '💲';
...
function handleEmojiCollision(emoji) {
    ...
    if (emojiText === '💣') {
        ...
        const randomPoints = Math.floor(Math.random() * 11) - 5; // -5 to +5
        const randomTime = Math.floor(Math.random() * 11) - 5; // -5 to +5
        score += randomPoints;
        time += randomTime;
    } else if (currentModifier === 'numbers') {
        const numberValue = parseInt(emojiText.replace(/️⃣/g, '')) || 0;
        score += numberValue;
        time += numberValue;
    } else {
        const goodEmojis = themeEmojis[currentTheme].filter(e => e !== '🧲');
        if (goodEmojis.includes(emojiText)) {
            const pointIndex = Math.floor(Math.random() * 3) + modes[currentMode].goodPoints[0];
            const timeIndex = Math.floor(Math.random() * 5) + modes[currentMode].goodTime[0];
            score += pointIndex;
            time += timeIndex;
        }
    }
