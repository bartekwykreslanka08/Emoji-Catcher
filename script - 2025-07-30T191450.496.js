...
const languages = {
    ...
    pl: {
        ...
        invalidLogin: 'Nieprawidłowa nazwa użytkownika lub hasło!',
        fillFields: 'Wypełnij wszystkie pola!',
        usernameExists: 'Ta nazwa użytkownika już istnieje!',
        registrationSuccess: 'Rejestracja udana! Zaloguj się.',
...
function login() {
    ...
    if (!username || !password) {
        alert(languages[currentLanguage].fillFields);
        return;
    }
    ...
    if (user) {
        ...
        document.getElementById('playerInfo').style.display = 'block';
        document.getElementById('startGameBtn').textContent = languages[currentLanguage].startGame;
        document.getElementById('modesBtn').textContent = languages[currentLanguage].modes;
        document.getElementById('indexBtn').textContent = languages[currentLanguage].index;
        ...
        alert(languages[currentLanguage].invalidLogin);
    }
}
...
function register() {
    ...
    if (users.find(u => u.username === username)) {
        alert(languages[currentLanguage].usernameExists);
    } else {
        users.push({ username, password });
        localStorage.setItem('users_' + location.hostname, JSON.stringify(users));
        alert(languages[currentLanguage].registrationSuccess);
...
function logout() {
    ...
    document.getElementById('mainSection').style.display = 'none';
    resetGame();
    document.getElementById('loginSection').querySelector('button:nth-child(3)').textContent = languages[currentLanguage].login;
    document.getElementById('loginSection').querySelector('button:nth-child(4)').textContent = languages[currentLanguage].register;
}
...
function startGame() {
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        document.getElementById('stopButton').textContent = languages[currentLanguage].exit;
        gameStarted = true;
        isPaused = false;
...
function endGame() {
    ...
    alert(`${languages[currentLanguage].gameOver}${score}`);
    resetGame();
    document.getElementById('mainSection').style.display = 'block';
    document.getElementById('loginSection').style.display = 'none';
}
...
function toggleModes() {
    ...
    document.getElementById('modesContent').querySelector('h2').textContent = languages[currentLanguage].selectMode;
    document.getElementById('levelSection').querySelector('h3').textContent = languages[currentLanguage].level;
    document.getElementById('themeSection').querySelector('h3').textContent = languages[currentLanguage].theme;
    document.getElementById('modifierSection').querySelector('h3').textContent = languages[currentLanguage].modifier;
    document.getElementById('modesContent').querySelector('button').textContent = languages[currentLanguage].close;
}

function toggleIndex() {
    const indexTab = document.getElementById('indexTab');
    indexTab.style.display = indexTab.style.display === 'flex' ? 'none' : 'flex';
    document.getElementById('indexContent').querySelector('button').textContent = languages[currentLanguage].close;
}
...
function changeLanguage() {
    ...
    localStorage.setItem('currentLanguage_' + location.hostname, currentLanguage);
    updateLanguage();
    if (localStorage.getItem('loggedInUser_' + location.hostname)) {
        document.getElementById('startGameBtn').textContent = languages[currentLanguage].startGame;
        document.getElementById('modesBtn').textContent = languages[currentLanguage].modes;
        document.getElementById('indexBtn').textContent = languages[currentLanguage].index;
    } else {
    ...
    document.querySelectorAll('.language-selector label')[0].textContent = languages[currentLanguage].language + ':';
    toggleModes();
    toggleIndex();
}
...
function updateLanguage() {
    ...
    document.querySelectorAll('.language-selector label')[0].textContent = languages[currentLanguage].language + ':';
    document.getElementById('loginSection').querySelector('label:nth-child(3)').textContent = languages[currentLanguage].username;
    document.getElementById('loginSection').querySelector('label:nth-child(4)').textContent = languages[currentLanguage].password;
    document.getElementById('loginSection').querySelector('label:nth-child(5)').textContent = languages[currentLanguage].saveCredentials;
}