...
function resetGame() {
    ...
    isPaused = false;
    score = 0;
    time = currentModifier === 'oneMinute' ? 60 : modes[currentMode].initialTime; // Zachowuje czas dla modyfikatora
    playerPosition = 35;
    targetPosition = 35;
    emojiCount = 0;
    magnetActive = false;
...
function setMode(mode) {
    ...
    updateGameModeInfo();
    localStorage.setItem('currentMode_' + location.hostname, currentMode); // Zapis trybu
}
...
function setTheme(theme) {
    ...
    applyTheme(theme);
    updateGameModeInfo();
    localStorage.setItem('currentTheme_' + location.hostname, currentTheme); // Zapis tematu
}
...
function setModifier(modifier) {
    ...
    updateGameModeInfo();
    localStorage.setItem('currentModifier_' + location.hostname, currentModifier); // Zapis modyfikatora
    if (gameStarted) resetGame(); // Resetuje grę, jeśli modyfikator zmieniono w trakcie gry
}
...
function checkCollisions() {
    ...
    emojis.forEach(emoji => {
        ...
        const collisionHeight = playerRect.height / 2;

        if (currentModifier === 'duet') {
            if ((Math.abs(emojiX - playerPosition) < collisionWidth &&
                 rect.bottom >= playerRect.top + collisionHeight / 2 &&
                 rect.top <= playerRect.bottom - collisionHeight / 2 &&
                 rect.bottom >= playerRect.top && rect.top <= playerRect.bottom) ||
                (secondPlayerRect && Math.abs(emojiX - secondPlayerPosition) < collisionWidth &&
                 rect.bottom >= secondPlayerRect.top + collisionHeight / 2 &&
                 rect.top <= secondPlayerRect.bottom - collisionHeight / 2 &&
                 rect.bottom >= secondPlayerRect.top && rect.top <= secondPlayerRect.bottom)) {
...
function handleMagnet() {
    if (magnetActive) {
        ...
        emojis.forEach(emoji => {
            ...
            if (distance < magnetRadius && verticalOverlap > playerRect.height / 4) {
                ...
                const collisionWidth = (currentModifier === 'biggerBasket' ? playerWidth * 1.5 : currentModifier === 'miniBasket' ? playerWidth / 2 : playerWidth) / 2;
                if (Math.abs(emojiX - playerPosition) < collisionWidth && verticalOverlap > playerRect.height / 2) {
                    handleCollision(emoji);
