...
const moveSpeed = 15;
const magnetRadius = 35; // Zwiększony promień dla lepszego efektu
const magnetPullStrength = 1.2; // Zwiększona siła dla płynności
...
function toggleTopScores() {
    ...
    topScoresSection.style.display = topScoresSection.style.display === 'flex' ? 'none' : 'flex';
    if (topScoresSection.style.display === 'flex') showTopScores();
}
                        ...
                        if (player || secondPlayer) {
                            ...
                            if (magnetActive) {
                                ...
                                if (minDistance <= magnetRadius) {
                                    const pullSpeed = magnetPullStrength * (1 - minDistance / magnetRadius) * deltaTime * 300;
                                    const newX = parseFloat(emoji.style.left) + (targetX - emojiX) * pullSpeed;
                                    emoji.style.left = `${Math.max(0, Math.min(gameWidth - 7, newX))}vw`;
                                    emoji.style.transition = 'left 0.05s linear'; // Płynna animacja
                                }
                            }
                            ...
                            if ((player && checkCollision(playerRect)) || (secondPlayer && checkCollision(secondPlayerRect))) {
                                ...
                                if (emojiData.type === 'magnet') {
                                    ...
                                    emoji.style.animation = 'pulseMagnet 0.5s infinite'; // Animacja magnesu
                                } else {
