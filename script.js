...
const modes = {
    easy: { initialTime: 50, goodPoints: [4, 5, 6, 5, 4, 6, 5, 4, 6, 5, 4, 6, 5], goodTime: [5, 6, 8, 6, 5, 7, 6, 5, 7, 6, 5, 8, 6], badPoints: [-5], badTime: [-6], emojiInterval: 800, fallDuration: 4667, bombChance: 0.2, magnetChance: 0.1 }, // 7000 / 1.5
    medium: { initialTime: 30, goodPoints: [5, 6, 7, 6, 5, 7, 6, 5, 7, 6, 5, 7, 6], goodTime: [6, 7, 9, 7, 6, 8, 7, 6, 8, 7, 6, 9, 7], badPoints: [-6], badTime: [-7], emojiInterval: 600, fallDuration: 3333, bombChance: 0.4, magnetChance: 0.08 }, // 5000 / 1.5
    hard: { initialTime: 15, goodPoints: [6, 7, 8, 7, 6, 8, 7, 6, 8, 7, 6, 9, 7], goodTime: [7, 8, 10, 8, 7, 9, 8, 7, 9, 8, 7, 10, 8], badPoints: [-7], badTime: [-8], emojiInterval: 400, fallDuration: 2000, bombChance: 0.6, magnetChance: 0.06 } // 3000 / 1.5
};
...
const magnetPullStrength = 2.5; // Zwiększona siła magnesu dla lepszego działania
...
function startGame() {
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        function gameLoopTimestamp(timestamp) {
            ...
            if (time > 0 || currentModifier === 'oneMinute') {
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis && time > 0) {
                ...
                if (random < bombChance) {
                    emojiText = '💣';
                } else if (random < bombChance + magnetChance) {
                    emojiText = '🧲';
                } else {
...
function updatePlayerPosition() {
    if (gameStarted && !isPaused) {
        if (isMovingLeft && !isMovingRight) {
            targetPosition = Math.max(0, playerPosition - moveSpeed * deltaTime * 20);
        }
        if (isMovingRight && !isMovingLeft) {
            targetPosition = Math.min(gameWidth - playerWidth, playerPosition + moveSpeed * deltaTime * 20);
        }
        if (currentModifier === 'duet') {
...
function handleEmojiCollision(emoji) {
    ...
    if (emojiText === '💣') {
        ...
        if (index !== -1) {
            score += modes[currentMode].goodPoints[index % modes[currentMode].goodPoints.length];
            time += modes[currentMode].goodTime[index % modes[currentMode].goodTime.length];
        }
    }
...
function handleMagnet() {
    if (magnetActive) {
        ...
        for (let emoji of emojis) {
            if (!emoji.classList.contains('magnet-effect')) { // Pomijamy sam magnes
                const emojiRect = emoji.getBoundingClientRect();
                const emojiX = (emojiRect.left + emojiRect.right) / 2 - gameAreaRect.left;
                const emojiY = (emojiRect.top + emojiRect.bottom) / 2 - gameAreaRect.top;
                const playerX = (playerRect.left + playerRect.right) / 2 - gameAreaRect.left;
                const playerY = (playerRect.top + playerRect.bottom) / 2 - gameAreaRect.top;

                const dx = playerX - emojiX;
                const dy = playerY - emojiY;
                const distance = Math.sqrt(dx * dx + dy * dy);

                if (distance < magnetRadius) {
                    const pullStrength = magnetPullStrength * (1 - distance / magnetRadius) * deltaTime * 60; // Normalizacja do płynności
                    const newX = emojiX + (dx / distance) * pullStrength;
                    const newY = emojiY + (dy / distance) * pullStrength;
                    emoji.style.left = `${Math.max(0, Math.min(gameWidth - 6, newX))}vw`;
                    emoji.style.top = `${Math.max(-6, Math.min(94, newY))}vh`; // Ograniczenie w granicach gry
                }
            }
        }
    }
...
function updateEmojiAnimation() {
    ...
    for (let emoji of emojis) {
        ...
        if (rect.bottom >= gameAreaRect.bottom) {
            ...
            currentTop += (100 / modes[currentMode].fallDuration) * deltaTime * 1000; // Płynne spadanie
...
function updateGameModeInfo() {
    document.getElementById('gameModeInfo').textContent = `Tryb: ${currentMode.charAt(0).toUpperCase() + currentMode.slice(1)}`;
    document.getElementById('themeInfo').textContent = `Motyw: ${currentTheme.charAt(0).toUpperCase() + currentTheme.slice(1)}`;
