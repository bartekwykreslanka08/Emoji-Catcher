...
const modes = {
    ...
    medium: { initialTime: 30, goodPoints: [5, 6, 7], goodTime: [6, 7, 9], badPoints: [-5], badTime: [-6], emojiInterval: 600, fallDuration: 2000, bombChance: 0.1, dollarChance: 0.02, zeroChance: 0.025 },
    hard: { initialTime: 15, goodPoints: [6, 7, 8], goodTime: [7, 8, 10], badPoints: [-5], badTime: [-6], emojiInterval: 400, fallDuration: 1500, bombChance: 0.15, dollarChance: 0.02, zeroChance: 0.025 },
    hardPlus: { initialTime: 10, goodPoints: [7, 8, 9], goodTime: [7, 9, 10], badPoints: [-7], badTime: [-7], emojiInterval: 300, fallDuration: 1200, bombChance: 0.175, dollarChance: 0.015, zeroChance: 0.02 }
};
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis && time > 0) {
                ...
                if (currentModifier === 'disco') {
                    if (random < 0.2) emojiText = '💣';
                    else if (random < 0.4) emojiText = '💲';
                    else if (random < 0.6) emojiText = '0️⃣';
                    else emojiText = '🪩';
