...
const modes = {
    easy: { initialTime: 50, waveTime: 20, goodPoints: [4, 5, 6, 5, 4, 6, 5, 4], goodTime: [5, 6, 8, 6, 5, 7, 6, 5], badPoints: [-5], badTime: [-6], emojiInterval: 800, bombChance: 0.3, magnetChance: 0.05 },
    medium: { initialTime: 30, waveTime: 15, goodPoints: [5, 6, 7, 6, 5, 7, 6, 5], goodTime: [6, 7, 9, 7, 6, 8, 7, 6], badPoints: [-6], badTime: [-7], emojiInterval: 600, bombChance: 0.4, magnetChance: 0.03 },
    hard: { initialTime: 15, waveTime: 10, goodPoints: [6, 7, 8, 7, 6, 8, 7, 6], goodTime: [7, 8, 10, 8, 7, 9, 8, 7], badPoints: [-7], badTime: [-8], emojiInterval: 400, bombChance: 0.5, magnetChance: 0.01 }
};
...
window.onload = () => {
    ...
    }, 2000); // Ekran ładowania trwa 2 sekundy
};
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis) {
                if (time > 0) {
                    ...
                    emoji.style.animationDuration = `${3 + Math.random() * 2}s`; // Różne prędkości spadania
                    ...
                    function animateEmoji() {
                        ...
                        const top = parseFloat(emoji.style.top || '-10') + (80 / parseFloat(emoji.style.animationDuration)) * deltaTime;
