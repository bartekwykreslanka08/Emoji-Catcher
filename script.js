...
const moveSpeed = 15;
const magnetRadius = 35; // Zwiększony promień magnesu
const magnetPullStrength = 0.9; // Zwiększona siła przyciągania
...
function toggleTopScores() {
    ...
    topScoresTab.style.display = topScoresTab.style.display === 'flex' ? 'none' : 'flex';
    if (topScoresTab.style.display === 'flex') {
        showTopScores();
    }
}
                        ...
                        if (player || secondPlayer) {
                            ...
                            if (magnetActive) {
                                ...
                                if (minDistance <= magnetRadius) {
                                    const pullSpeed = magnetPullStrength * (1 - minDistance / magnetRadius) * deltaTime * 300; // Zwiększona częstotliwość i płynność
                                    const newX = parseFloat(emoji.style.left) + (targetX - emojiX) * pullSpeed;
                                    emoji.style.left = `${Math.max(0, Math.min(gameWidth - 7, newX))}vw`;
                                    emoji.style.transition = 'left 0.05s linear'; // Płynne przesuwanie
                                }
                            }
                            ...
                            if ((player && checkCollision(playerRect)) || (secondPlayer && checkCollision(secondPlayerRect))) {
                                ...
                                if (emojiData.type === 'magnet') {
                                    ...
                                    emoji.style.animation = 'pulseMagnet 0.5s infinite'; // Efekt wizualny magnesu
                                } else {
...
function toggleIndex() {
    ...
    if (indexTab.style.display === 'flex') {
