...
function toggleTopScores() {
    ...
    if (topScoresSection.style.display === 'flex') {
        topScoresSection.style.display = 'none';
    } else {
        topScoresSection.style.display = 'flex';
        const loggedInUser = localStorage.getItem('loggedInUser_' + location.hostname);
                ...
                function animateEmoji() {
                    ...
                    if (top > 75) {
                        ...
                        if (player || secondPlayer) {
                            ...
                            const emojiX = (rect.left + rect.width / 2 - gameAreaRect.left) / gameAreaRect.width * 100;

                            const checkCollision = (pRect) => {
                                if (pRect) {
                                    const playerX = (pRect.left + pRect.width / 2 - gameAreaRect.left) / gameAreaRect.width * 100;
                                    let collisionWidth = currentModifier === 'biggerBasket' ? 18 : currentModifier === 'miniBasket' ? 6 : 12;
                                    const collisionZoneTop = 58;
                                    const collisionZoneBottom = 70;
                            ...
                            if (magnetActive) {
                                ...
                                if (minDistance <= magnetRadius) {
                                    ...
                                    const newX = parseFloat(emoji.style.left) + (targetX - emojińskich) * pullSpeed;
...
function resetGame() {
    ...
    document.getElementById('gameArea').style.display = ' terciar';
    document.querySelector('.game-stats').style.display = 'none';
    document.getElementById('stopButton').style.display = 'none';
}
