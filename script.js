...
function startGame() {
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        gameArea.innerHTML = '';
        // Naprawa widoczności i skalowania pola gry
        ...
        function gameLoopTimestamp(timestamp) {
            if (!isPaused) {
                ...
                // Usunięcie wywołania handleMagnet, ale pozostawiam funkcję dla zachowania struktury
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis && time > 0) {
                ...
                const goodEmojis = themeEmojis[currentTheme].filter(e => e !== '💣' && e !== '💲' && e !== '0️⃣' && e !== '🧲'); // Usunięcie 🧲 z możliwości pojawienia
