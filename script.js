...
const modes = {
    easy: { initialTime: 50, goodPoints: [4, 5, 6], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 800, fallDuration: 3000, bombChance: 0.15, dollarChance: 0.02, zeroChance: 0.03 },
    medium: { initialTime: 30, goodPoints: [5, 6, 7], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 600, fallDuration: 2000, bombChance: 0.3, dollarChance: 0.02, zeroChance: 0.03 },
    hard: { initialTime: 15, goodPoints: [6, 7, 8], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 400, fallDuration: 1500, bombChance: 0.45, dollarChance: 0.02, zeroChance: 0.03 },
    hardPlus: { initialTime: 10, goodPoints: [7, 8, 9], goodTime: [5, 6, 7, 8, 9], badPoints: [-5], badTime: [-6], emojiInterval: 300, fallDuration: 1200, bombChance: 0.6, dollarChance: 0.02, zeroChance: 0.03 }
};
...
window.onload = () => {
    ...
    document.getElementById('gameArea').addEventListener('touchstart', handleTouchStart, { passive: true });
    document.getElementById('gameArea').addEventListener('touchmove', handleTouchMove, { passive: true });
    document.getElementById('gameArea').addEventListener('touchend', handleTouchEnd, { passive: true });
...
function startGame() {
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        function gameLoopTimestamp(timestamp) {
            if (!isPaused) {
                deltaTime = Math.min((timestamp - lastTime) / 1000, 0.016); // Cel: 60 FPS (16.67ms)
                lastTime = timestamp;
                time -= deltaTime;
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis && time > 0) {
                ...
                const dollarChance = modes[currentMode].dollarChance;
                const zeroChance = modes[currentMode].zeroChance;
                ...
                if (currentModifier === 'disco') {
                    ...
                    if (random < bombChance) {
                        emojiText = '💣';
                    } else if (random < bombChance + dollarChance) {
                        emojiText = '💲';
                    } else if (random < bombChance + dollarChance + zeroChance) {
                        emojiText = '0️⃣';
                    } else {
                        emojiText = themeEmojis[currentTheme][Math.floor(Math.random() * themeEmojis[currentTheme].length)];
                    }
                }
                emoji.textContent = emojiText;
                emoji.style.left = Math.random() * (gameWidth - 6) + 'vw';
                emoji.style.top = '-6vh';
                emoji.style.transition = `top ${modes[currentMode].fallDuration / 1000}s linear`;
                emoji.style.willChange = 'transform'; // Optymalizacja renderowania
...
function handleTouchMove(e) {
    if (gameStarted && !isPaused) {
        ...
        targetPosition = Math.max(0, Math.min(gameWidth - playerWidth, playerPosition + deltaX * 0.5)); // Wygładzenie ruchu
        touchStartX = touchX;
    }
}
...
function updatePlayerPosition() {
    if (gameStarted && !isPaused) {
        ...
        if (currentModifier === 'duet') {
            ...
            document.querySelector('.second-player').style.transform = `translateX(${secondPlayerPosition}vw) translateZ(0)`; // Przyspieszenie GPU
        }
        playerPosition = targetPosition;
        document.querySelector('.player').style.transform = `translateX(${playerPosition}vw) translateZ(0)`; // Przyspieszenie GPU
    }
}
...
function handleEmojiCollision(emoji) {
    ...
    if (emojiText === '💣') {
        ...
        const numberValue = parseInt(emojiText.replace(/️⃣/, '')) || 0;
        score += numberValue;
        time += numberValue;
    } else {
        ...
        for (let emoji of emojis) {
            if (!emoji.classList.contains('magnet-effect')) {
                ...
                if (emojiY >= playerY && emojiY <= playerY + magnetHeight) {
                    ...
                    const pullStrength = 10 * deltaTime * 60;
                    const newX = emojiX + dx * pullStrength;
                    emoji.style.transform = `translateX(${Math.max(0, Math.min(gameWidth - 6, newX))}vw)`; // Użycie transform dla płynności
                }
            }
        }
    }
...
function updateEmojiAnimation() {
    ...
    for (let emoji of emojis) {
        ...
        if (rect.bottom >= gameAreaRect.bottom) {
            ...
            emoji.style.transform = `translateY(${currentTop}vh)`; // Użycie transform dla płynności
        }
    }
}
