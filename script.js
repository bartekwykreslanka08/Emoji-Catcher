...
let deltaTime = 0;
let emojiCount = 0;
let magnetActive = false; // Zachowano dla kompatybilności, ale nie używane
let magnetTimeout;
let currentModifier = localStorage.getItem('currentModifier_' + location.hostname) || 'none';
let secondPlayerPosition = 55;

const modes = {
    easy: { initialTime: 50, goodPoints: [4, 5, 6, 5, 4, 6, 5, 4, 6, 5, 4, 6, 5], goodTime: [5, 6, 8, 6, 5, 7, 6, 5, 7, 6, 5, 8, 6], badPoints: [-5], badTime: [-6], emojiInterval: 800, fallDuration: 3000, bombChance: 0.15, dollarChance: 0.02, zeroChance: 0.03 },
    medium: { initialTime: 30, goodPoints: [5, 6, 7, 6, 5, 7, 6, 5, 7, 6, 5, 7, 6], goodTime: [6, 7, 9, 7, 6, 8, 7, 6, 8, 7, 6, 9, 7], badPoints: [-5], badTime: [-6], emojiInterval: 600, fallDuration: 2000, bombChance: 0.3, dollarChance: 0.015, zeroChance: 0.025 },
    hard: { initialTime: 15, goodPoints: [6, 7, 8, 7, 6, 8, 7, 6, 8, 7, 6, 9, 7], goodTime: [7, 8, 10, 8, 7, 9, 8, 7, 9, 8, 7, 10, 8], badPoints: [-5], badTime: [-6], emojiInterval: 400, fallDuration: 1500, bombChance: 0.45, dollarChance: 0.01, zeroChance: 0.02 },
    hardPlus: { initialTime: 10, goodPoints: [7, 8, 9, 8, 7, 9, 8, 7, 9, 8, 7, 10, 8], goodTime: [8, 9, 11, 9, 8, 10, 9, 8, 10, 9, 8, 11, 9], badPoints: [-5], badTime: [-6], emojiInterval: 300, fallDuration: 1200, bombChance: 0.6, dollarChance: 0.008, zeroChance: 0.015 }
};
...
function startGame() {
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        function gameLoopTimestamp(timestamp) {
            ...
            if (gameStarted) gameLoop = requestAnimationFrame(gameLoopTimestamp);
        }
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis && time > 0) {
                ...
                const goodEmojis = themeEmojis[currentTheme].filter(e => !['💣', '💲', '0️⃣'].includes(e));
                ...
                emoji.style.fontSize = '6.5vh'; // Rozmiar emoji dopasowany do koszyka
                document.getElementById('gameArea').appendChild(emoji);
                emojiCount++;
                lastSpawn = timestamp;
            }
            if (time > 0 && gameStarted) emojiSpawnLoop = requestAnimationFrame(spawnEmoji);
        }
...
function handleEmojiCollision(emoji) {
    ...
    if (emojiText === '💣') {
        ...
        if (index !== -1) {
            const pointIndex = Math.floor(Math.random() * 3) + 4; // Losowe 4-6
            const timeIndex = Math.floor(Math.random() * 5) + 5; // Losowe 5-9
            score += pointIndex;
            time += timeIndex;
        }
    }
