...
const modes = {
    ...
    medium: { initialTime: 30, goodPoints: [5, 6, 7, 6, 5, 7, 6, 5, 7, 6, 5, 7, 6], goodTime: [6, 7, 9, 7, 6, 8, 7, 6, 8, 7, 6, 9, 7], badPoints: [-5], badTime: [-6], emojiInterval: 600, fallDuration: 2000, bombChance: 0.4, magnetChance: 0.08 },
    hard: { initialTime: 15, goodPoints: [6, 7, 8, 7, 6, 8, 7, 6, 8, 7, 6, 9, 7], goodTime: [7, 8, 10, 8, 7, 9, 8, 7, 9, 8, 7, 10, 8], badPoints: [-5], badTime: [-6], emojiInterval: 400, fallDuration: 1500, bombChance: 0.6, magnetChance: 0.06 }
};
...
function startGame() {
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        function gameLoopTimestamp(timestamp) {
            if (!isPaused) {
                deltaTime = (timestamp - lastTime) / 1000; // Precyzyjna kontrola czasu
                lastTime = timestamp;
                time -= deltaTime;
...
function handleEmojiCollision(emoji) {
    ...
    if (emojiText === '💣') {
        ...
        magnetActive = true;
        score += 10; // +10 punktów za magnes zgodnie z indexem
