...
const modes = {
    easy: { initialTime: 50, goodPoints: [4, 5, 6], goodTime: [5, 6, 8], badPoints: [-5], badTime: [-6], emojiInterval: 800, fallDuration: 3000, bombChance: 0.15, magnetChance: 0.02, zeroChance: 0.025, dollarChance: 0.02 },
    medium: { initialTime: 30, goodPoints: [5, 6, 7], goodTime: [6, 7, 9], badPoints: [-5], badTime: [-6], emojiInterval: 600, fallDuration: 2000, bombChance: 0.3, magnetChance: 0.02, zeroChance: 0.025, dollarChance: 0.02 },
    hard: { initialTime: 15, goodPoints: [6, 7, 8], goodTime: [7, 8, 10], badPoints: [-5], badTime: [-6], emojiInterval: 400, fallDuration: 1500, bombChance: 0.45, magnetChance: 0.02, zeroChance: 0.025, dollarChance: 0.02 },
    hardPlus: { initialTime: 10, goodPoints: [7, 8, 9], goodTime: [8, 9, 11], badPoints: [-5], badTime: [-6], emojiInterval: 300, fallDuration: 1000, bombChance: 0.6, magnetChance: 0.02, zeroChance: 0.025, dollarChance: 0.02 }
};
...
function handleEmojiCollision(emoji) {
    ...
    if (emojiText === '💣') {
        ...
        const randomPoints = Math.floor(Math.random() * 31) - 10; // -10 do +20
        const randomTime = Math.floor(Math.random() * 21) - 10; // -10 do +10
        score += randomPoints;
        time += randomTime > 0 ? randomTime : Math.min(randomTime, -1); // Minimalnie -1s dla grywalności
