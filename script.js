...
const modes = {
    easy: { initialTime: 50, waveTime: 20, goodPoints: [4, 5, 6, 5, 4, 6, 5, 4], goodTime: [5, 6, 8, 6, 5, 7, 6, 5], badPoints: [-5], badTime: [-6], emojiInterval: 600, bombChance: 0.3, magnetChance: 0.05 },
    medium: { initialTime: 30, waveTime: 15, goodPoints: [5, 6, 7, 6, 5, 7, 6, 5], goodTime: [6, 7, 9, 7, 6, 8, 7, 6], badPoints: [-6], badTime: [-7], emojiInterval: 450, bombChance: 0.4, magnetChance: 0.03 },
    hard: { initialTime: 15, waveTime: 10, goodPoints: [6, 7, 8, 7, 6, 8, 7, 6], goodTime: [7, 8, 10, 8, 7, 9, 8, 7], badPoints: [-7], badTime: [-8], emojiInterval: 300, bombChance: 0.5, magnetChance: 0.01 }
};
...
function backToMain() {
    ...
    document.getElementById('waveTimer').textContent = modes[currentMode].waveTime;
    document.getElementById('gameArea').classList.remove('fullscreen');
    document.querySelector('.game-stats').style.display = 'none';
    document.getElementById('closeButton').style.display = 'none';
}
...
function startGame() {
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        waveTimer = modes[currentMode].waveTime;
        ...
        if (currentModifier === 'duet') {
            gameArea.innerHTML += '<div class="player" style="transform: translateX(30vw);">🧺</div>';
            gameArea.innerHTML += '<div class="second-player" style="transform: translateX(50vw);">🧺</div>';
            playerPosition = 30;
        ...
        function gameLoopTimestamp(timestamp) {
            if (!isPaused) {
                ...
                if (!isWaveActive && currentModifier !== 'oneMinute') {
                    ...
                    document.getElementById('waveTimer').textContent = Math.max(0, Math.round(waveTimer * 10) / 10);
                    if (waveTimer <= 0) {
                        startWave();
                    }
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis) {
                if (time > 0) {
                    ...
                    const magnetChance = modes[currentMode].magnetChance * (isWaveActive ? 0.5 : 1);
                    const emojis = [
                        ...
                        function animateEmoji() {
                            ...
                            if (magnetActive && emojiData.type !== 'bad') {
                                ...
                                if (player || secondPlayer) {
                                    ...
                                    const pullStrength = 0.8; // Zwiększona siła przyciągania

                                    if (playerRect) {
                                        const playerX = (playerRect.left - gameAreaRect.left) / gameAreaRect.width * 100;
                                        const targetX = playerX - (playerWidth / (currentModifier === 'biggerBasket' ? 1.5 : 2)) + (playerWidth * Math.random() / (currentModifier === 'biggerBasket' ? 1.5 : 2));
                                        const dx = (targetX - parseFloat(emoji.style.left)) * pullStrength * deltaTime * 60;
                                        emoji.style.left = (parseFloat(emoji.style.left) + dx) + 'vw';
                                    }
                                    if (secondPlayerRect) {
                                        const secondPlayerX = (secondPlayerRect.left - gameAreaRect.left) / gameAreaRect.width * 100;
                                        const targetX2 = secondPlayerX - (playerWidth / 2) + (playerWidth * Math.random());
                                        const dx2 = (targetX2 - parseFloat(emoji.style.left)) * pullStrength * deltaTime * 60;
                                        emoji.style.left = (parseFloat(emoji.style.left) + dx2) + 'vw';
                                    }
                                }
                            }
...
function startWave() {
    isWaveActive = true;
    waveTimer = modes[currentMode].waveTime * 1.5; // Wydłużona fala
    document.getElementById('waveTimer').textContent = Math.round(waveTimer);
    setTimeout(() => {
        isWaveActive = false;
        waveTimer = modes[currentMode].waveTime;
        document.getElementById('waveTimer').textContent = waveTimer;
