...
function startGame() {
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        document.querySelector('.game-stats').style.display = 'block'; // Zawsze na górze podczas gry
        document.getElementById('stopButton').style.display = 'inline-block'; // Zawsze na górze podczas gry
        gameStarted = true;
        isPaused = false;
...
function endGame() {
    ...
    // Nie otwieraj automatycznie zakładki z wynikami, wróć do ekranu głównego
    backToMain();
}
