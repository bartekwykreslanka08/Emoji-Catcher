...
function startGame() {
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        function gameLoopTimestamp(timestamp) {
            if (!isPaused) {
                deltaTime = Math.min(0.1, (timestamp - lastTime) / 1000); // Ograniczenie deltaTime dla stabilności
                lastTime = timestamp;
                time -= deltaTime;
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= (currentModifier === 'disco' ? 500 : modes[currentMode].emojiInterval) && emojiCount < maxEmojis && time > 0) {
                ...
                const bombChance = currentModifier === 'disco' ? 0 : modes[currentMode].bombChance;
                const dollarChance = currentModifier === 'disco' ? 0 : modes[currentMode].dollarChance;
                const zeroChance = currentModifier === 'disco' ? 0 : modes[currentMode].zeroChance;
...
function handleEmojiCollision(emoji) {
    ...
    setTimeout(() => {
        emoji.remove();
        emojiCount--;
    }, 50); // Szybsze usuwanie dla płynności
    document.getElementById('score').textContent = score;
    document.getElementById('time').textContent = Math.max(0, Math.round(time * 10) / 10);
}
