...
const fallDuration = 5000; // 5 sekund na pełne spadanie
    ...
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        function gameLoopTimestamp(timestamp) {
            if (!isPaused) {
                ...
                checkCollisions();
                updateEmojiAnimation(timestamp); // Aktualizacja animacji emoji
            }
        ...
        function spawnEmoji(timestamp) {
            if (!isPaused && timestamp - lastSpawn >= modes[currentMode].emojiInterval && emojiCount < maxEmojis && time > 0) {
                ...
                emojiCount++;
                setTimeout(() => emoji.classList.add('active'), 10); // Rozpoczęcie animacji z lekkim opóźnieniem
...
function handleCollision(emoji) {
    ...
    if (emoji.parentElement) {
        ...
        }, 300); // Zgodne z czasem zanikania opacity w CSS
    }
}
...
function updateEmojiAnimation(timestamp) {
    ...
    emojis.forEach(emoji => {
        ...
        const yOffset = -10 + progress * (gameAreaHeight + 10); // Od -10vh (góra) do height + 10vh (dół)
