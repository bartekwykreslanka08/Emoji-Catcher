...
function startGame() {
    if (!gameStarted && localStorage.getItem('loggedInUser_' + location.hostname)) {
        ...
        document.querySelector('.game-stats').style.display = 'block'; // Natychmiast na górze od startu gry
        document.getElementById('stopButton').style.display = 'inline-block'; // Natychmiast na górze od startu gry
        gameStarted = true;
        isPaused = false;
...
function endGame() {
    ...
    gameStarted = false;
    document.getElementById('gameArea').style.display = 'none';
    // Zachowaj .game-stats i #stopButton widoczne do wyświetlenia wyników
    ...
    showTopScores(); // Otwórz zakładkę z wynikami po zakończeniu gry
}
...
function setModifier(modifier) {
    ...
    if (modifier === 'oneMinute') {
        time = 60;
        document.getElementById('time').textContent = time;
    } else if (modifier === 'biggerBasket' || modifier === 'duet' || modifier === 'miniBasket') {
        // Brak zmiany czasu, tylko wizualna/adaptacyjna
    } else {
