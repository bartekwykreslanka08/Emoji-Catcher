...
function checkCollisions() {
    ...
    emojis.forEach(emoji => {
        ...
        if (currentModifier === 'duet') {
            if ((Math.abs(emojiX - playerPosition) < collisionWidth && rect.bottom >= playerRect.top && rect.top <= playerRect.bottom) ||
                (secondPlayerRect && Math.abs(emojiX - secondPlayerPosition) < collisionWidth && rect.bottom >= secondPlayerRect.top && rect.top <= secondPlayerRect.bottom)) {
                handleCollision(emoji);
            }
        } else if (Math.abs(emojiX - playerPosition) < collisionWidth && rect.bottom >= playerRect.top && rect.top <= playerRect.bottom) {
            handleCollision(emoji);
        }
    });
}
...
function handleMagnet() {
    if (magnetActive) {
        ...
        emojis.forEach(emoji => {
            ...
            if (distance < magnetRadius && rect.bottom >= playerRect.top && rect.top <= playerRect.bottom) {
...
function updateEmojiAnimation(timestamp) {
    ...
    emojis.forEach(emoji => {
        ...
        const yOffset = -10 + progress * (gameAreaHeight + 10);
